...
function testSetRowsData2() {
  var sheetIdx = 1;
  data = [ {name: "Janko", age: "33"}, {name: "Mojca", age: "28"} ];
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheets()[sheetIdx];
  setRowsData(sheet, data);
}

function testSetRowsData() {
  var sheetIdx = 1;
  data = [ [ "Janko", 33 ], [ "Mojca", 28 ] ];
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheets()[sheetIdx];
  //setRowsData(sheet, data);
  Browser.msgBox(sheet.getSheetName());
  var destinationRange = sheet.getRange(1, 1, data.length, data[0].length);
